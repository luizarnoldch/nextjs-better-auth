        ...
        <Breadcrumb>
          <BreadcrumbList>
            {segments.flatMap((segment, idx) => {
              ...
              const raw =
                segmentReplacements[segment] ?? segment.charAt(0).toUpperCase() + segment.slice(1).replace(/-/g, ' ');
              const href = hrefs[idx];
              const items = [
                <BreadcrumbItem key={href}>
                  {isLast ? <BreadcrumbPage>{raw}</BreadcrumbPage> : <BreadcrumbLink href={href}>{raw}</BreadcrumbLink>}
                </BreadcrumbItem>,
              ];
              if (!isLast) {
